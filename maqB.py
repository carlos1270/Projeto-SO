...
class ThreadMaquinas(threading.Thread):
    ...
    def run(self):
        ...
        # Mensagem/requisicao da maquina
        info = get_info(informacao.decode())
        # Indica que a thread está na região critica e bloqueia as demais
        semaforo.acquire()
        if(info[0] == 0):
...
def verificarDisponibilidade():
    ...
    while maquinaEncontrada == False:
        if(nucleos[1][0] == False):
            ...
            return controlarBufferNucleos('172.17.0.3', 2, 1)
        elif(nucleos[1][1] == False):
            maquinaEncontrada = True
            return controlarBufferNucleos('172.17.0.3', 2, 2)
